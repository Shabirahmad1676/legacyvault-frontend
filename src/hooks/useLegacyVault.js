...
export function useVaultItems() {
  return useQuery({
    ...
    queryFn: async () => data(await vault.getVaultItems()),
  });
}
export function useCreateVaultItem() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: vault.createVaultItem,
...
export function useUpdateVaultItem() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, body }) => vault.updateVaultItem(id, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["vault-items"] }),
  });
}
export function useDeleteVaultItem() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: vault.deleteVaultItem,
    onSuccess: () => qc.invalidateQueries({ queryKey: ["vault-items"] }),
  });
}
export function useTrustedContacts() {
  return useQuery({
    ...
    queryFn: async () => data(await contacts.getTrustedContacts()),
  });
}
export function useAddTrustedContact() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: contacts.addTrustedContact,
    onSuccess: () => {
...
export function useRemoveTrustedContact() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: contacts.removeTrustedContact,
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["trusted-contacts"] });
...
export function useIncomingAccessRequests() {
  return useQuery({
    ...
    queryFn: async () => data(await requests.getIncomingRequests()),
  });
}
export function useAccessRequestsToVote() {
  return useQuery({
    queryKey: ["access-requests-to-vote"],
...
export function useVoteOnAccessRequest() {
  ...
  return useMutation({
    mutationFn: ({ requestId, decision }) =>
      votes.castVote(requestId, decision),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["access-requests-to-vote"] });
      qc.invalidateQueries({ queryKey: ["incoming-access-requests"] });
...
export function useActivityLogs() {
  return useQuery({
    ...
    queryFn: async () => data(await activity.getActivityLogs()),
  });
}
export function useUpdateQuorum() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: auth.updateQuorum,
